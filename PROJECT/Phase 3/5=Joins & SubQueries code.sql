...
(SupplierID, SupplierName, ContactNumber, Supplies)
 VALUES
(1, 'FitnessGear', '03101234567', 'Treadmills'),
(2, 'IronWorks', '03107654321', 'Dumbbells'),
(3, 'BenchMasters', '03105554443', 'Benches'),
(4, 'YogaEssentials', '03102233445', 'Mats'),
(5, 'BikePro', '03106677889', 'Bikes'),
(6, 'RowingExperts', '03109988777', 'Rowers'),
(7, 'EllipticalWorld', '03100011223', 'Ellipticals'),
(8, 'KettleKing', '03103332211', 'Kettlebells'),
(9, 'ResistancePlus', '03104445566', 'Bands'),
(10, 'PullUpPros', '03107776555', 'Pull-up Bars');
...
-- Q: 1   INNER JOIN
SELECT m.FirstName, m.LastName, c.ClassName, f.Rating, f.Comments, t.FirstName , t.LastName 
FROM Member AS m
INNER JOIN Attendance AS a ON m.MemberID = a.MemberID
INNER JOIN Class AS c ON a.ClassID = c.ClassID
INNER JOIN Feedback AS f ON m.MemberID = f.MemberID AND c.ClassID = f.ClassID
INNER JOIN Trainer AS t ON c.TrainerID = t.TrainerID;


-- Q: 2   LEFT JOIN


SELECT m.FirstName, m.LastName, c.ClassName, t.FirstName, t.LastName, a.AttendanceDate
FROM Member AS m
LEFT JOIN Attendance AS a ON m.MemberID = a.MemberID
LEFT JOIN Class AS c ON a.ClassID = c.ClassID
LEFT JOIN Trainer t ON c.TrainerID = t.TrainerID;


-- Q: 3   
SELECT c.ClassName, c.Schedule, t.FirstName, t.LastName , m.FirstName, m.LastName,f.Rating,f.Comments
FROM Attendance AS a
RIGHT JOIN Class AS c ON a.ClassID = c.ClassID
RIGHT JOIN Member AS m ON a.MemberID = m.MemberID
RIGHT JOIN Trainer t ON c.TrainerID = t.TrainerID
LEFT JOIN Feedback f ON c.ClassID = f.ClassID AND m.MemberID = f.MemberID;


-- Q: 4   Full join
SELECT m.memberID, m.FirstName, NULL AS TrainerID, NULL AS TrainerFirstName, p.MembershipId, p.MembershipType, NULL AS ClassID, NULL AS ClassName
FROM MEMBER AS m
LEFT JOIN Membership AS p  ON m.MembershipID=p.MembershipID
UNION
SELECT NULL AS memberID, NULL AS FirstName, t.TrainerId, t.FirstName, NULL AS MembershipId, NULL AS MembershipType, c.ClassId, c.Classname
FROM Trainer AS t
RIGHT JOIN Class  c on t.TrainerId= c.trainerId;





-- Retrieve the names of members who attended classes instructed by trainers id  '5'.
...
-- Retrieve the names of members who have attended at least one class instructed by trainers specializing in 'Cardio'.
...
WHERE EXISTS (
    ...
    AND t.Specialization = 'Bodybuilding'
);


-- Retrieve the names of all members who have attended classes scheduled on '2024-06-15' 
...
WHERE MemberID IN (
    ...
    JOIN Class c ON a.ClassID = c.ClassID
    JOIN Trainer t ON c.TrainerID = t.TrainerID
    WHERE c.Schedule >= '2024-06-15' AND c.Schedule < '2024-06-16'
);
